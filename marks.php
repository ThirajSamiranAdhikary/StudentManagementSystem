...
if(isset($_POST['submit'])){
  ...
  $sql = "insert into student_subject values('".$sid."','".$subID."','".$mark."')";
  $sql2="select * from student_subject where studentID='{$sid}' and subjectID='{$subID}' limit 1";
  $sql3="select * from user where UserName='{$sid}' limit 1";
  $sql4="select * from subject where subjectID='{$subID}' limit 1";
  $re=mysqli_query($con,$sql2);
  $re2=mysqli_query($con,$sql3);
  $re3=mysqli_query($con,$sql4);

 
      
      if(mysqli_num_rows($re)==1){
        $errormsg="<p>Subject ID $sid and $subID already exist. So not insert into database</p>";
      }

      elseif(mysqli_num_rows($re2)!=1){
        $errormsg="<p>$sid is an invalid User</p>";
      }

      elseif(mysqli_num_rows($re3)!=1){
        $errormsg="<p>$subID is an invalid subject ID</p>";
      }
    ...
    <div class="row">
        <div class="col-md-2" style="background-color: black; opacity: 0.7; height: 100%; border-radius: 5px;">
            <div class="d-flex align-items-start">
                ...
                <button class="nav-link active" id="v-pills-messages-tab" data-bs-toggle="pill" data-bs-target="#v-pills-messages" type="button" role="tab" aria-controls="v-pills-messages" aria-selected="false" onClick="marksFunction()"><b>Add Marks</b></button>
    ...
    <div class="col-md-9" >
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" id="sn" name="subID" required>
              </div>
        ...
        <div class="row">
            <div class="col-md-4">
                <label for="inputEmail4" class="form-label" style="color: aliceblue;">Marks:</label>
                <input type="number" class="form-control" id="cd" name="mark"  min=0 max=100 required>
              </div>
